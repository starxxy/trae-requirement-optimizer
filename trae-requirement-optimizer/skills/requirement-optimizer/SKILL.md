...
description: "模拟 Trae Code 内置的'优化您的输入'功能：对用户指令进行结构化分析、按需提问澄清、确认后执行。当用户提出新的开发/实现型需求时，自动调用本技能进行指令优化与确认。Simulates Trae Code's built-in 'Optimize Your Input': structurally analyzes user instructions, asks clarifying questions only when needed, and executes only after confirmation. Automatically invoked for new development or implementation requests."
---
